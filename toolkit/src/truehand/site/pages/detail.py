...
def _render_expertise_link_block(label, entries):
    ...
    if not entries:
        return ""
    links = ", ".join(f'<a href="{ent.href}">{html.escape(ent.name)}</a>' for ent in entries)
    return (
...
def detail_page_generic(
    e, list_href, list_label, link_map, session_lookup=None, graph=None, relations=None
):
    ...
    meta_block = f'<aside class="meta-block">{"".join(meta_rows)}</aside>' if meta_rows else ""

    sessions_block = _render_mentioned_in(e.meta["sessions"].many(), session_lookup or {})
  ...
  {linked}
    ...
    return page(
        e.name,
        body,
        current_nav=list_href,
        breadcrumb=bc,
        description=e.summary or e.body,
        image=e.image,
        canonical=e.href,
        og_type="article",
    )
    ...
    def links_for(adj, rel):
        ...
        for r, other_id in adj:
            ...
            if other and other["url"]:
                out.append(f'<a href="{other["url"]}">{html.escape(other["name"])}</a>')
        return out
    ...
    for direction, rel, label in spec:
        if direction == "faction":
            ...
            for r, fid in graph.out_adj.get(href, []):
                ...
                members = [
                    graph.node_by_id[m]
                    for m in graph.faction_members.get(fid, [])
