#!/usr/bin/env python3
"""Turn a session fact-check worksheet into an interactive audio-review page.

Reads   sessions/<DATE>/transcript-distilled.factcheck.md
Writes  sessions/<DATE>/transcript-distilled.factcheck.html   (next to it)

Every [HH:MM:SS] timestamp in the evidence becomes a clickable link that seeks
the session recording (recording.m4a, referenced by relative path) to that
moment and plays it, so an attribution can be verified against the audio in one
click. The ✔ / ✘ column becomes a three-state toggle (unset → ✔ → ✘), and each
row has a correction box — marking a row ✘ opens it automatically. Verdicts and
corrections persist in the browser and can be copied back as a ready-to-apply
list.

The page must be opened from the session folder (so the sibling recording.m4a
resolves); it is intentionally NOT a website artifact — it is a local QA tool,
a companion to the markdown worksheet (which stays the source of record).

Stdlib only. Usage:  python3 scripts/generate-factcheck-review.py YYYY-MM-DD
"""
import html
import pathlib
import re
...
def main() -> None:
    if len(sys.argv) != 2:
        sys.exit("usage: generate-factcheck-review.py YYYY-MM-DD")
    date = sys.argv[1]
    root = pathlib.Path(__file__).resolve().parent.parent
    src = root / "sessions" / date / "transcript-distilled.factcheck.md"
    if not src.exists():
        sys.exit(f"no worksheet at {src}")
    md = src.read_text(encoding="utf-8")
    body = render(md)
    out = src.with_suffix(".html")  # transcript-distilled.factcheck.html
    out.write_text(PAGE.format(date=date, body=body), encoding="utf-8")
    rec = src.parent / "recording.m4a"
    note = "" if rec.exists() else "  (warning: recording.m4a not found beside it)"
